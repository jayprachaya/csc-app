...
that Normal, Mild, Moderate or Severe.''')
    ...
    if save_pdf.button('Save'):
        ...
        pdf.ln(5)
        pdf.cell(page_width, h = 5.0, align = 'L', txt = 'Total Severity Score(TSS): ' + str(st.session_state.TSS), ln = 1)
        pdf.ln(5)
        ...
        with NamedTemporaryFile(delete=False, suffix=".png") as tmpfile,NamedTemporaryFile(delete=False, suffix=".jpg") as rawimg:
            ...
            pdf.image(rawimg.name, x = 45, y = 170, w = 50, h = 50)
            pdf.image(tmpfile.name, x = 85, y = 170, w = 50, h = 50)
        with NamedTemporaryFile(delete=False, suffix=".png") as tmpfile,NamedTemporaryFile(delete=False, suffix=".jpg") as rawimg:
            if st.session_state.raw_img3d.shape[0] <= 175:
            ...
            fig.savefig(tmpfile.name)
            pdf.image(rawimg.name, x = 45, y = 210, w = 50, h = 50)
            pdf.image(tmpfile.name, x = 85, y = 210, w = 50, h = 50)
        with NamedTemporaryFile(delete=False, suffix=".png") as tmpfile,NamedTemporaryFile(delete=False, suffix=".jpg") as rawimg:
            if st.session_state.raw_img3d.shape[0] <= 175:
            ...
            fig.savefig(tmpfile.name)
            pdf.image(rawimg.name, x = 45, y = 250, w = 50, h = 50)
            pdf.image(tmpfile.name, x = 85, y = 250, w = 50, h = 50)
